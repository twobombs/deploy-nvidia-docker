...
RUN export DEBIAN_FRONTEND=noninteractive && apt-get install -y  --install-recommends xfce4 xfce4-goodies ubuntu-desktop cuda-samples-8-0 gnome-panel gnome-settings-daemon metacity nautilus gnome-terminal libsdl1.2-dev mesa-utils && cd /usr/local/cuda-8.0/samples && make && make all && make install && apt-get clean all

...
RUN apt-get update && export DEBIAN_FRONTEND=noninteractive && apt-get install -y software-properties-common && add-apt-repository -y ppa:wine/wine-builds && dpkg --add-architecture i386 && apt-get upgrade -y && apt-get install -y git net-tools nvidia-375 qemu-kvm qemu virt-manager virt-viewer libvirt-bin && apt-get clean all

RUN export DEBIAN_FRONTEND=noninteractive && apt-get install -y  --install-recommends wine-staging xfce4 xfce4-goodies ubuntu-desktop gnome-panel gnome-settings-daemon metacity nautilus gnome-terminal libsdl1.2-dev mesa-utils && apt-get clean all
